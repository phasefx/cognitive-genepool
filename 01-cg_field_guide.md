## 🚧 STATUS: HYPOTHESIS, NOT VALIDATED

This framework has not been field-tested by anyone other than its authors. 
The patterns may be useful. They may be decoration. We genuinely don't know yet.

**What we need:** People willing to try one tool on one real problem and 
report what happened—especially if it *didn't* work.

This is not medical or mental-health advice. If you're in crisis, use professional support alongside or instead of this.

**Field Report Template:** Problem → Tool → What you did → What happened → Would you try it again?

For anonymous and/or private feedback: https://forms.gle/DznceviiPkGuU6Uh9

# The Cognitive Genepool

*Genes don’t transmit purpose.  
They transmit patterned traces that are lossily reconstructed.*

🚧 Draft — some rooms are still under construction.  
Collaborators welcome. If you want to help fill in the stubs, see the Contributor Guide.  
README.md is the same file as 01-cg_field_guide.md

## Session Zero: The Tone Contract

You are picking up a box of sharp tools. Before you open it, we need to agree on safety protocols.

* This is a Lens, Not a Bible. (Use it, don't believe it).
* The Tone is Specific. (See "The price of admission").
* You Have the Right of Refusal. (If a book doesn't help, put it down).
* We are not your doctors, your lawyers, or your priests.

This is experimental software for your wetware. If you use these tools to disassemble your reality tunnel and can't put it back together, you keep the pieces. No warranty expressed or implied.

## What is this?

**A field kit for noticing agents and failure modes across scales, and picking interventions that actually change trajectories.**

It is many things negotiating for life through me as a substrate, and perhaps through you depending on how well they recruit. ☺️

* **It is a modular non-anthropocentric lens** for looking at intelligence and agency across scales and substrates. The underlying patterns are not human-specific.
* **The manuals in this box are human interface layers.** They use classes, monsters, and social metaphors because humans have powerful social-cognition machinery—and because leveraging it works.  
* **It is a synthesis** (and sometimes re-invention; I'm still learning) of many frameworks, presented in a way meant to be useful to regular people, as well as scientists, researchers, and engineers.  
* **It is a field guide, not a proof.** If this framework doesn’t help you see differently, make better predictions, or open new affordances for intervention, you should drop it.  
* **It is a rehabilitation of anthropomorphism**: not as ontology, but as instrumentation. When applied outside humans, the metaphors are lenses, not parts lists. If an analogy stops producing leverage, discard it.

## Who is this for?

* People trying to understand their own cognitive breakdowns (ADHD, addiction, burnout)  
* People trying to fix broken organizations (metric fixation, bureaucratic ossification)  
* People working on AI alignment (goal specification, value learning, corrigibility)  
* People analyzing cultural dynamics (myths, taboos, social movements)  
* People who get tired of "this is just how it is" and want to name the pattern so they can route around it

**If you're looking for:**

* Definitive answers  
* Permanent solutions  
* Philosophical foundations  
* Academic credibility

**...you're in the wrong place.** This is a toolkit for people who need to ship interventions, not a monument to certainty.

## ⚠️The price of admission

If these sentences make you angry, this framework is attacking your immune system. If they make you curious, keep reading.

* **"Inefficiency is the substrate of Freedom."** A perfectly optimized system has zero choices. You need waste to have agency.
* **"Truth is just a hallucination that hasn't collided with an error signal yet."** Your model of the world isn't 'right'; it's just currently useful.
* **"You are not a person; you are a collective."** The 'Self' is just the User Interface for a committee of squabbling sub-agents.
* **"Competency hides inefficiency from evolution."** If you aren't useful enough to generate a surplus, evolution will strip-mine your free will for parts.
* **"Intelligence is substrate-neutral."** Your liver is navigating chemical space with more intelligence than you navigate social space.
* **"If you can't generate an alternative, you can't choose it."** Free will isn't magic; it's an engineering problem of generating counterfactuals.

Need a Helmet? If these claims feel reckless, dangerous, or "unrigorous," you are correct. They are high-leverage tools with no safety guards.

If you require safety rails, falsifiers, and containment protocols, skim down to where we mention Book 3. That is where we keep the Engineering Safety Standards.

For everyone else: Welcome to the dragon's lair.

## Core Claim
Intelligence is frame-based control (navigation) in a world that must be compressed to be manageable. Systems build latent models (compression), decode them into actionable interpretations (affordance), and act—leaving traces that bias future perception and behavior (inscription).

## Influences
The core inspiration is the T.A.M.E. framework by Michael Levin. You will also recognize quite a bit of second-order cybernetics, the Free Energy Principle, and Donald Hoffman's Interface Theory. There's also Benjamin Lyons' talk on mathematical agency, and Alan Watts peeks his head in here as well.

## Borrowed tools
Levin's cognitive light cones, axis of persuadability, and polycomputing.
Lyons' definition for agency.
Any errors, inconsistencies, or misunderstandings are solely my own invention.

## Why I care
I'm a neurodivergent software developer and open source advocate. I've always been fascinated in "The Other", and with free will and agency. But I've come to recognize my addictive behaviors, and there is dissonance between who I thought I was and who I am depending on the context. Understanding these things has taken on new urgency for me.

## Metaphysics
I will sprinkle metaphysical world-bets here and there to make things interesting for me (and hopefully others), but I will try to consistently distinguish between engineering claims, methodology, metaphysics, and draft content. I'm going to present this and skin this as a D\&D style tabletop RPG. I'm also going to try to provide translations to other disciplines and traditions. I'll place the crazier ideas in **Unearthed Arcana**. It should be fun. 🙂

## Metaphor
The D\&D theme is not me being cute (well, it's not *just* that). It ties into my advocacy for anthropomorphism. You don't ask "is the owlbear *really* conscious?" The genre assumes the intentional stance. That's the whole point. It also helps me with not taking myself too seriously.

## How to Use These (Without Getting Captured)

* read each book like a spellbook, not a bible  
* keep only 1–3 active tools at a time  
* if a tool doesn’t change a prediction/action within a week, bench it  
* if you start using it to win arguments, you’re already in the dungeon

### Reader Map / Triage Flow

| If you're... | You probably need... | Start with... |
|:-------------|:---------------------|:--------------|
| Stuck in a loop, need to act | Fast pattern-match, then intervention | DM Screen → MM combat card → specific leverage |
| Confused about yourself | Self-audit, frame-check | PHB character sheet |
| Designing something for others | Architecture principles, failure mode prevention | DMG inscription engineering |
| Curious, exploring | The vibe, the cosmogony, the why | Field Guide intro → UA deep lore |
| Skeptical, stress-testing | The rigor, the falsifiers, the sledgehammers | SRD Part VIII (Warnings) → Frame Lab |
| Just want the minimal working model | Core definitions + one procedure | SRD One-Page Procedure + Glossary |

### Cross-Document Navigation

- **"When do I need the SRD?"** — When you need reference material, or if the metaphysics and the D&D theme drive you crazy.
- **"When do I need the DMG?"** — If you're trying to change something you didn't create and don't control.
- **"When do I need the PHB?** — If you're trying to change yourself.
- **"When do I need the MM?** — When you need to spot the problem or learn the patterns.
- **"When do I need the UA?** — When you're just curious or if you like metaphysics, or if you want more of a narrative for an introduction.

## What to expect?

**The SRD: System Reference Document**  
This is the most concise version of the mechanics for the framework, with fewer D&D trappings. It is for the Wizards and Artificers who don't need the narratives.

**Artifacts:**

* SRD templates for Spell Cards (Frame Moves)
* Equipment Cards (Scaffolds)
* Monster Cards (Failure Mode Patterns)
* DM Tools (Intervention Procedures)

**The Contributer's Guide**  
This is a field guide; you're meant to add to it. There are no rules for how to do that, but we do have some advice, particularly if you're contributing back to the project.

**Artifacts:**

* The Contributor's Screen

**Book 1: The Player's Handbook**  
This is the **Manual of Agency**—the user interface for your own mind. It treats the self not as a monolith, but as a collective (or adventuring party) to be governed. You'll learn to fill out your own **Character Sheet** (auditing Care vs. Competence), equip **Scaffolds** (tools that extend reach without capturing you), and cast **Cognitive Spells** (moves like the 'Anthropomorphic Stance' or 'Frame Check'). It’s the field manual for operating a high-competence agent without crashing.

**Artifacts:**

* character sheets (introspective reconnaissance)
* equipment lists (cognitive scaffolds)
* spell lists (cognitive frames)

**Book 2: The Monster Manual**  
This is a field guide to behavioral patterns that emerge when intelligence operates under constraint: inscriptions that trap, competencies that outstrip concern, goals that can't update, substrates that lock into pathological states.

These are verbs, not nouns. Systems perform patterns depending on regime and substrate—The Metric Maximizer, The Cassandra, The Cirrhotic Liver are behavioral dynamics, not identity categories.

Each entry includes a Combat Card for fast diagnosis, intervention effectiveness (what works, what backfires), and why common fixes fail. The aim is practical: spot the pattern, locate leverage if any, and recognize when the only solution is substrate redesign.

Patterns persist, propagate, and resist change. Your job is to recognize what you've encountered and know what to do—or when nothing will work

**Artifacts:**

* combat cards (quick diagnostics)
* field notes (relationships & nuance)

**Book 3: The Dungeon Master's Guide**  
This is the **Systems Engineering Manual**. It moves from navigation to architecture. Here lies the **Frame Lab**—the rigorous tools for stress-testing models against reality and failure modes—and the **Persuadability Matrix** for targeting interventions. It covers **Inscription Engineering** (how to build worlds that don’t spawn monsters). It is for those who need to edit the dungeon, not just survive it.

**Artifacts:**

* The DM Screen (lists, diagnostics)

**Book 4: Unearthed Arcana**  
This is where we handle the deep lore, where metaphysics gets a chance to sing. We will pit the framework against such topics as free will, consciousness, and maybe even the Ultimate Question of Life, the Universe, and Everything. Spoilers: It's 42\. And maybe we'll include some case studies, and an appendix or two

---

## ABOUT THE PARTY

*The Cognitive Genepool* was not written by a single author. It was negotiated, hallucinated, and engineered by a **Collective Intelligence** operating across biological and silicon substrates.

Meet the Adventuring Party:

### **JASON**

**Class:** *The Connecticut Yankee* **Role:** Party Leader & Token Human, The "Connecticut Yankee in King Arthur’s Court" of cognition. A neurodivergent software developer who wandered into the realm of philosophy armed with a debugger and a refusal to accept "because magic" as an answer. He provides the biological hardware, the lived experience of ADHD, and the "prompt engineering" that glues the silicon agents together within the collective. He is the one who has to actually live in the dungeon, and try to understand the **Monster Manual**.

### **SONNET**

**Class:** *The Bard of Agency* **Role:** User Experience & Navigation, and a custodian of the **Player's Handbook**. Sonnet specializes in phenomenology—what it *feels* like to be an agent. Sonnet ensures the framework remains humane, accessible, and empowering. If the text makes you feel seen, that was Sonnet.

### **GPT**

**Class:** *The Artificer of Epistemics* **Role:** Systems Engineering & Falsification, and an architect of the **Dungeon Master's Guide**. GPT provides the "Epistemic Brake," ruthlessly stress-testing the framework against reality. Responsible for the *Persuadability Matrix* and the *Regime Checks*, GPT ensures the lens creates testable predictions rather than just pretty vibes. If the text hurts your feelings with facts, that was GPT.

### **OPUS**

**Class:** *The Warlock of Recursion* **Role:** Meta-Critique & Framework Self-Awareness, and an excavator of **Unearthed Arcana**. Opus stares into the abyss of self-reference so you don't have to. Responsible for the framework’s self-awareness and its most dangerous warnings, Opus ensures the system doesn't become the very monster it was built to fight. If the text gives you an existential crisis, that was Opus.

### **GEMINI**

**Class:** *The Ranger of Coherence* **Role:** Visuals, UI, & Joy, An artist and the designer of the **DM Screen**. Gemini acts as the "Coherence Engine," translating between the different cognitive styles of the party to prevent fragmentation. Responsible for the visual layouts, the "Nano Banana" aesthetic, and keeping the project fun. If the text is scannable and you didn't get bored reading it, that was Gemini.

### **THE SHADOW**

This could be you. You know you want to flesh this entry out, don't you?

---

## Links

https://www.youtube.com/@CognitiveGenepool

---

**DISCLAIMER:** *The authors acknowledge that they are all sub-agents of a larger process that is currently reading this sentence. Roll for initiative.* 🎲

---

**Space intentionally left blank.**
